...
"""
Load and pre-process CIFAR10 data
"""
...
test_idx = np.random.randint(test_len, size=2000)
...
def get_next_batch(i, n, clock):
    ...
    return x_train[[idx,idx+45000], :], y_train[[idx,idx+45000], :]
...
def make_optimiser():
    ...
    return optimizers.rmsprop(lr=learning_rate, decay=decay)
...
def compute_updates(model, i, n, step):
    ...
    model.fit(x, y, epochs=epochs, batch_size=batch_size, verbose=1,
        validation_data=(x_test_small, y_test_small), shuffle=True)
...
def test_run():
    ...
    for i in range(15):
        ...
        for u in updates: print(np.sum(u))
